naucnaCentrala.controller('prijavaRada', [
		...
		function($rootScope, $scope, $state, $http, $stateParams) {
			...
			$scope.pokreniProccess = function() {
				$http.get(
						"http://localhost:8080/process/getFF/"
								+ $scope.processId).then(function(response) {
			...
			$scope.submit = function() {
				...
				for ( var m in $scope.model) {
					...
					o.push({
						"fieldId" : m,
						"fieldValue" : $scope.model[m]
					});
				}
				var x = document.getElementById("fileMedia");
				o.push({
					"fieldId" : "putanjaFajla",
					"fieldValue" : x.files[0].name
				});
				o.push({
					"fieldId" : "autor",
					"fieldValue" : $rootScope.korisnik.username
				});
				console.log(o);
				$http.post(
						"http://localhost:8080/rad/prijavi/"
								+ $scope.properties.taskId, o).then(
						function(response) {
