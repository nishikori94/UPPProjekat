naucnaCentrala
		.controller(
				...
				[
						...
						'FileSaver',
						'Blob',
						function($rootScope, $scope, $state, $http, FileSaver,
								Blob) {
							...
							$scope.preuzmiRad = function(y) {
								...
								$http
										.post(
												"http://localhost:8080/rad/downloadNaziv",
												y.rad,
												{
							...
							$scope.posaljiRad = function(task) {
								var putanja = document
										.getElementById("fileMedia").files[0].name;
								$http
										.post(
												"http://localhost:8080/rad/ispraviRad/"
														+ putanja, task)
										.then(
												function(response) {
