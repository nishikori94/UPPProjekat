naucnaCentrala.controller('odaberiRecenzente', [
		'$rootScope',
		'$scope',
		'$state',
		'$http',
		...
		function($rootScope, $scope, $state, $http, $stateParams) {
			...
			$scope.prikaziCasopise = function() {
				$http
						.get(
								"http://localhost:8080/recenzent/sviRecenzenti/"
			...
			$scope.potvrdiRecenzente = function() {
				$http.post(
						"http://localhost:8080/recenzent/odabraniRecenzenti/"
								+ task, $scope.odabrani).then(
						function(response) {
